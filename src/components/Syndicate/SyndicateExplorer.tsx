    ...
    <div className="space-y-6">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-xs text-muted-foreground max-w-md mx-auto">
          Вступи в пакт. Выживи вместе или заработай на слабых.
        </p>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-xs text-muted-foreground">
                {mySyndicate.members.filter(m => m.status === 'ACTIVE').length} активных членов
              </div>
            </div>
            <div className="text-right">
              <div className="text-xl font-bold text-secondary">${mySyndicate.totalPool}</div>
              <div className="text-xs text-muted-foreground">общий пул</div>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="card-terminal p-3 text-center">
          ...
          <div className="text-xs text-muted-foreground uppercase">Активных синдикатов</div>
        </div>
        <div className="card-terminal p-3 text-center">
          <div className="text-2xl font-bold text-secondary">
            ${syndicates.reduce((sum, s) => sum + s.totalPool, 0)}
          </div>
          <div className="text-xs text-muted-foreground uppercase">Всего на кону</div>
        </div>
        <div className="card-terminal p-3 text-center">
          <div className="text-2xl font-bold text-destructive">
            {syndicates.reduce((sum, s) => sum + s.members.filter(m => m.status === 'ELIMINATED').length, 0)}
          </div>
          <div className="text-xs text-muted-foreground uppercase">Элиминировано</div>
        </div>
      </div>
      ...
      {availableSyndicates.length === 0 && !mySyndicate && (
        <div className="card-terminal p-8 text-center">
          ...
          <p className="text-muted-foreground">Синдикаты недоступны.</p>
          <p className="text-xs text-muted-foreground mt-2">Загляни позже.</p>
        </div>
      )}
