    ...
    <div className="card-terminal p-4 space-y-4 hover:border-primary/50 transition-all">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">{daysRemaining} дней осталось</p>
        </div>
        <div className={`px-2 py-1 text-xs uppercase ${
          ...
          {syndicate.status === 'ACTIVE' ? 'АКТИВЕН' : syndicate.status}
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="text-center p-2 bg-muted/30 border border-border">
          ...
          <div className="text-xs text-muted-foreground uppercase">Пул</div>
        </div>
        ...
        <div className="text-center p-2 bg-muted/30 border border-border">
          ...
          <div className="text-xs text-muted-foreground uppercase">Отсев</div>
        </div>
        ...
        <div className="text-center p-2 bg-muted/30 border border-border">
          ...
          <div className="text-xs text-muted-foreground uppercase">Награда</div>
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <span>ГЛАДИАТОРЫ ({activeMembers.length}/{syndicate.members.length})</span>
        </div>
      ...
      <div className="p-3 bg-energy/5 border border-energy/30">
        <div className="flex items-center justify-between">
          <span className="text-xs uppercase text-muted-foreground">Потенциальный заработок</span>
          <span className="text-lg font-bold text-energy">
            +${estimatedBounty - 50}
          </span>
        </div>
        <p className="text-xs text-muted-foreground mt-1">
          На основе {attritionRate.toFixed(0)}% прогноза отсева
        </p>
      </div>
