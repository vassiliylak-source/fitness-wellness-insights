...
const BreathingExercises = () => {
  ...
  const getPhaseColor = () => {
    ...
    switch (breathingState.phase) {
      case 'inhale': return baseColor === 'blue' ? 'blue' : baseColor === 'green' ? 'green' : baseColor === 'purple' ? 'purple' : 'indigo';
      case 'hold': return 'yellow';
      case 'exhale': return baseColor === 'blue' ? 'cyan' : baseColor === 'green' ? 'emerald' : baseColor === 'purple' ? 'violet' : 'blue';
      case 'pause': return 'gray';
      default: return baseColor;
    }
  };
  return <div className="space-y-6">
      <Card className={`bg-gradient-to-r ${currentTechnique.color === 'blue' ? 'from-blue-50 to-cyan-50 border-blue-200' : 
        currentTechnique.color === 'green' ? 'from-green-50 to-emerald-50 border-green-200' :
        currentTechnique.color === 'purple' ? 'from-purple-50 to-violet-50 border-purple-200' :
        ...
        <CardHeader className="text-center">
          <div className="flex items-center justify-center gap-4 mb-2">
            <CardTitle className="flex items-center justify-center gap-2 text-2xl text-gray-800">
              <Wind className={`h-6 w-6 ${currentTechnique.color === 'blue' ? 'text-blue-600' : 
                currentTechnique.color === 'green' ? 'text-green-600' :
                currentTechnique.color === 'purple' ? 'text-purple-600' :
                'text-indigo-600'}`} />
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                {Object.entries(BREATHING_TECHNIQUES).map(([key, technique]) => (
                  <DropdownMenuItem key={key} onClick={() => changeTechnique(key as BreathingTechnique)}>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="flex justify-center">
            <div className="relative w-64 h-64 flex items-center justify-center">
              <div className={`w-32 h-32 rounded-full border-4 transition-all duration-1000 ease-in-out ${
                ...
                getPhaseColor() === 'emerald' ? 'border-emerald-500 bg-emerald-100' :
                getPhaseColor() === 'purple' ? 'border-purple-500 bg-purple-100' :
                getPhaseColor() === 'violet' ? 'border-violet-500 bg-violet-100' :
                getPhaseColor() === 'indigo' ? 'border-indigo-500 bg-indigo-100' :
                getPhaseColor() === 'yellow' ? 'border-yellow-500 bg-yellow-100' :
                ...
                <div className="w-full h-full flex items-center justify-center">
                  <div className="text-center">
                    <div className={`text-2xl font-bold ${
                      getPhaseColor() === 'blue' ? 'text-blue-700' :
                      getPhaseColor() === 'cyan' ? 'text-cyan-700' :
                      getPhaseColor() === 'green' ? 'text-green-700' :
                      getPhaseColor() === 'emerald' ? 'text-emerald-700' :
                      getPhaseColor() === 'purple' ? 'text-purple-700' :
                      getPhaseColor() === 'violet' ? 'text-violet-700' :
                      getPhaseColor() === 'indigo' ? 'text-indigo-700' :
                      getPhaseColor() === 'yellow' ? 'text-yellow-700' :
                      'text-gray-700'
                    }`}>
                      {breathingState.count}
                    </div>
                    <div className={`text-sm font-medium ${
                      getPhaseColor() === 'blue' ? 'text-blue-600' :
                      getPhaseColor() === 'cyan' ? 'text-cyan-600' :
                      getPhaseColor() === 'green' ? 'text-green-600' :
                      getPhaseColor() === 'emerald' ? 'text-emerald-600' :
                      getPhaseColor() === 'purple' ? 'text-purple-600' :
                      getPhaseColor() === 'violet' ? 'text-violet-600' :
                      getPhaseColor() === 'indigo' ? 'text-indigo-600' :
                      getPhaseColor() === 'yellow' ? 'text-yellow-600' :
                      'text-gray-600'
                    }`}>
                      {getPhaseDisplay()}
                    </div>
              ...
              {breathingState.isActive ? <>
                  <span className={`${
                    ...
                    {breathingState.phase === 'inhale' ? 'Breathe In' :
                     breathingState.phase === 'hold' ? 'Hold Your Breath' :
          ...
          <div className="flex justify-center gap-4">
            <Button onClick={toggleBreathing} className={`flex items-center gap-2 ${
              ...
              currentTechnique.color === 'green' ? 'bg-green-500 hover:bg-green-600' :
              currentTechnique.color === 'purple' ? 'bg-purple-500 hover:bg-purple-600' :
          ...
          <div className={`bg-white rounded-lg p-6 border ${
            currentTechnique.color === 'blue' ? 'border-blue-200' :
            currentTechnique.color === 'green' ? 'border-green-200' :
            currentTechnique.color === 'purple' ? 'border-purple-200' :
            'border-indigo-200'
          }`}>
            ...
            <ol className="list-decimal list-inside space-y-2 text-gray-700 text-sm">
              ...
              {currentTechnique.holdCount > 0 && (
                <li><strong>Hold your breath:</strong> Hold gently for {currentTechnique.holdCount} counts</li>
              )}
              <li><strong>Exhale slowly:</strong> Breathe out slowly through your nose or mouth for {currentTechnique.exhaleCount} counts</li>
              {breathingState.technique === 'box' && (
                <li><strong>Pause:</strong> Rest for {currentTechnique.holdCount} counts before the next inhale</li>
              )}
          ...
          <div className={`bg-gradient-to-r ${
            currentTechnique.color === 'blue' ? 'from-blue-50 to-white border-blue-200' :
            currentTechnique.color === 'green' ? 'from-green-50 to-white border-green-200' :
            currentTechnique.color === 'purple' ? 'from-purple-50 to-white border-purple-200' :
